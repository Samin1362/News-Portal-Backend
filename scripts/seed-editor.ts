/**
 * One-shot script: create or update a test editor account.
 *
 * Usage (from backend/):
 *   npx tsx scripts/seed-editor.ts                 # uses defaults
 *   EDITOR_EMAIL=foo@bar.test EDITOR_PASSWORD=hunter2 npx tsx scripts/seed-editor.ts
 *
...
const EMAIL = (process.env.EDITOR_EMAIL ?? 'editor.test@deligo.dev').toLowerCase();
const DISPLAY_NAME = process.env.EDITOR_DISPLAY_NAME ?? 'Editor (Test)';
const PASSWORD =
...
async function main(): Promise<void> {
  ...
  // 2. Mongo user — insert or update with role=editor.
  ...
  try {
    ...
    if (existing) {
      ...
      console.log(`[mongo] updated user ${existing._id.toString()} → role=editor`);
    } else {
      const doc = {
        _id: new ObjectId(),
        firebaseUid: uid,
      ...
      console.log(`[mongo] inserted user ${doc._id.toString()} (role=editor)`);
    }
  } finally {
    await client.close();
  }
...
main().catch((err) => {
  console.error('seed-editor failed:', err);
  process.exit(1);
});
